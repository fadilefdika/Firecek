        ...
        <div class="col-lg-10">
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Master Data - Schedule Type</h5>
                    <button type="button" class="btn btn-sm btn-primary rounded-pill px-3 fw-bold" 
                            style="font-size: 0.75rem;" data-bs-toggle="modal" data-bs-target="#modalScheduleType">
                        <i class="fas fa-plus me-1"></i> Add Type
                    </button>
                                    </div>
                <div class="card-body">
                    <table class="table table-bordered table-striped" id="schedule-type-table">
...
<div class="modal fade" id="modalScheduleType" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm"> <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-header border-bottom-0 pb-0">
                <h6 class="fw-bold mb-0">Add Schedule Type</h6>
                <button type="button" class="btn-close small" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.schedule-type.store') }}" method="POST">
                @csrf
                <div class="modal-body py-3">
                    <div class="mb-2">
                        <label class="form-label small fw-bold text-muted">Schedule Name</label>
                        <input type="text" name="schedule_name" class="form-control form-control-sm border-2" 
...
<div class="modal fade" id="modalEditType" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-header border-bottom-0 pb-0">
                <h6 class="fw-bold mb-0">Edit Schedule Type</h6>
                <button type="button" class="btn-close small" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="form-edit-type" method="POST">
                ...
                <div class="modal-body py-3">
                    ...
                    <div class="mb-2">
                        <label class="form-label small fw-bold text-muted">Schedule Name</label>
                        <input type="text" id="edit-name" name="schedule_name" class="form-control form-control-sm border-2" required>
                    </div>
                </div>
                <div class="modal-footer border-top-0 pt-0">
                    <button type="button" class="btn btn-sm btn-light fw-bold" data-bs-dismiss="modal">Cancel</button>
...
<script>
    $(function () {
        ...
        const table = $('#schedule-type-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin.schedule-type.data') }}',
            columns: [
        ...
        $('body').on('click', '.edit-type', function () {
            ...
            $('#form-edit-type').attr('action', `/admin/schedule-type/${id}`);
